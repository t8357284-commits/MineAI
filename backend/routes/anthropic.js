...
// ─── Helper ───────────────────────────────────────────────
async function callAnthropic(messages, systemPrompt, maxTokens = 1000) {
...
router.post('/chat', messageValidation, async (req, res, next) => {
  ...
  try {
    ...
    const data = await callAnthropic(messages, system, max_tokens || 800);
...
كن مباشراً وعملياً. لا مقدمات غير ضرورية.`;
    ...
    const data = await callAnthropic([{ role: 'user', content: prompt }], null, 1000);
...
أجب بـ JSON فقط:
{
  ...
  "tip": "نصيحة مهمة"
    ...
    const data = await callAnthropic([{ role: 'user', content: prompt }], null, 400);
    const raw = data.content?.map(c=>c.text||'').join('').replace(/```json|```/g,'').trim();
    const parsed = JSON.parse(raw);
...
JSON فقط: {"caption": "نص الكابشن الكامل", "hashtags": ["#1","#2","#3","#4","#5"]}`;
    ...
    const data = await callAnthropic([{ role: 'user', content: prompt }], null, 500);
    const raw = data.content?.map(c=>c.text||'').join('').replace(/```json|```/g,'').trim();
    const parsed = JSON.parse(raw);
